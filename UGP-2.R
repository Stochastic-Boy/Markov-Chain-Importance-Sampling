...
data <- log(data)
plot(x = x, y = data[,1], "l",col = "red",lwd="2",main=TeX("Performance with $\\hat{\\rho}_Y$ vs KDE estimate of $Y$"), xlab = "Number of Samples",ylab = TeX("$\\log($System time)"))
lines(x = x, y = data[,2], "l",col = "blue",lwd="2")
legend("bottomright", legend = c(TeX("$\\hat{\\rho}_Y$"),TeX("KDE estimate of $Y$")),col = c("red","blue"),lwd = 2.2)
